// -----------------------------------------
// Global definitions
// ----------------------------------------

// Header
#include "Constraints.h"

// Constants
//
const TableItemConstraint NVConstraint[DATA_TABLE_NV_SIZE] =
{
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 0
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 1
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 2
	{COEF_K_MIN, COEF_K_MAX, COEF_K_UG_DEF},									// 3
	{COEF_B_MIN, COEF_B_MAX, 0},												// 4
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 5
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 6
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 7
	{COEF_K_MIN, COEF_K_MAX, COEF_K_UPOT_DEF},									// 8
	{COEF_B_MIN, COEF_B_MAX, 0},												// 9
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 10
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 11
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 12
	{COEF_K_MIN, COEF_K_MAX, COEF_K_IMEAS_DEF},									// 13
	{COEF_B_MIN, COEF_B_MAX, 0},												// 14
	{COEF_RSH_MIN, COEF_RSH_MAX, COEF_RSH_CH0_DEF},								// 15
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 16
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 17
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 18
	{COEF_K_MIN, COEF_K_MAX, COEF_K_IMEAS_DEF},									// 19
	{COEF_B_MIN, COEF_B_MAX, 0},												// 20
	{COEF_RSH_MIN, COEF_RSH_MAX, COEF_RSH_CH1_DEF},								// 21
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 22
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 23
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 24
	{COEF_K_MIN, COEF_K_MAX, COEF_K_IMEAS_DEF},									// 25
	{COEF_B_MIN, COEF_B_MAX, 0},												// 26
	{COEF_RSH_MIN, COEF_RSH_MAX, COEF_RSH_CH2_DEF},								// 27
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 28
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 29
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 30
	{COEF_K_MIN, COEF_K_MAX, COEF_K_IMEAS_DEF},									// 31
	{COEF_B_MIN, COEF_B_MAX, 0},												// 32
	{COEF_RSH_MIN, COEF_RSH_MAX, COEF_RSH_CH3_DEF},								// 33
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 34
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 35
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 36
	{COEF_K_MIN, COEF_K_MAX, COEF_K_IMEAS_DEF},									// 37
	{COEF_B_MIN, COEF_B_MAX, 0},												// 38
	{COEF_RSH_MIN, COEF_RSH_MAX, COEF_RSH_CH4_DEF},								// 39
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 40
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 41
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 42
	{COEF_K_MIN, COEF_K_MAX, COEF_K_IMEAS_DEF},									// 43
	{COEF_B_MIN, COEF_B_MAX, 0},												// 44
	{COEF_RSH_MIN, COEF_RSH_MAX, COEF_RSH_CH5_DEF},								// 45
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 46
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 47
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 48
	{COEF_K_MIN, COEF_K_MAX, COEF_K_IMEAS_DEF},									// 49
	{COEF_B_MIN, COEF_B_MAX, 0},												// 50
	{COEF_RSH_MIN, COEF_RSH_MAX, COEF_RSH_CH6_DEF},								// 51
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 52
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 53
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 54
	{COEF_K_MIN, COEF_K_MAX, COEF_K_IMEAS_DEF},									// 55
	{COEF_B_MIN, COEF_B_MAX, 0},												// 56
	{COEF_RSH_MIN, COEF_RSH_MAX, COEF_RSH_CH7_DEF},								// 57
	{0, 0, 0},																	// 58
	{0, INT16U_MAX, COEF_BASE_VOLT_DEF},										// 59
	{0, INT16U_MAX, 0},															// 60
	{COEF_P2_MIN, COEF_P2_MAX, COEF_P2_DEF},									// 61
	{COEF_P1_MIN, COEF_P1_MAX, COEF_P1_DEF},									// 62
	{COEF_P0_MIN, COEF_P0_MAX, COEF_P0_DEF},									// 63
	{COEF_K_MIN, COEF_K_MAX, COEF_K_USET_DEF},									// 64
	{COEF_B_MIN, COEF_B_MAX, 0},												// 65
	{0, INT16U_MAX, U_SELTTEST_DEF},											// 66
	{0, INT16U_MAX, U_SELTTEST_DEF},											// 67
	{U_RTH_MIN, U_RTH_MAX, U_RTH_DEF},											// 68
	{COEF_I_CH0_MIN, INT16U_MAX, COEF_I_CH0_MIN},								// 69
	{COEF_I_CH1_MIN, INT16U_MAX, COEF_I_CH1_MIN},								// 70
	{COEF_I_CH2_MIN, INT16U_MAX, COEF_I_CH2_MIN},								// 71
	{COEF_I_CH3_MIN, INT16U_MAX, COEF_I_CH3_MIN},								// 72
	{COEF_I_CH4_MIN, INT16U_MAX, COEF_I_CH4_MIN},								// 73
	{COEF_I_CH5_MIN, INT16U_MAX, COEF_I_CH5_MIN},								// 74
	{COEF_I_CH6_MIN, INT16U_MAX, COEF_I_CH6_MIN},								// 75
	{COEF_I_CH7_MIN, INT16U_MAX, COEF_I_CH7_MIN},								// 76
	{0, INT16U_MAX, 0},															// 77
	{0, INT16U_MAX, 0},															// 78
	{0, INT16U_MAX, 0},															// 79
	{0, INT16U_MAX, 0},															// 80
	{0, INT16U_MAX, 0},															// 81
	{0, INT16U_MAX, 0},															// 82
	{0, COEF_THRESHOLD_MAX, 0},													// 83
	{0, COEF_THRESHOLD_MAX, 0},													// 84
	{U_ERR_COUNT_MIN, U_ERR_COUNT_MAX, U_ERR_COUNT_DEF},						// 85
	{0, INT16U_MAX, COEF_FLATTOP_DURATION_DEF},									// 86
	{0, COEF_THRESHOLD_MAX, 0},													// 87
	{0, INT16U_MAX, 0},															// 88
	{REGULATOR_TIME_MIN, REGULATOR_TIME_MAX, REGULATOR_TIME_MIN},				// 89
	{COEF_SWITCH_TIME_MIN, INT16U_MAX, COEF_SWITCH_TIME_MIN},					// 90
	{COEF_SWITCH_TIME_IGES_MIN, INT16U_MAX, COEF_SWITCH_TIME_IGES_MIN},			// 91
	{COEF_SWITCH_TIME_MIN, INT16U_MAX, COEF_SWITCH_TIME_MIN},					// 92
	{SLEW_RATE_MIN, INT16U_MAX, SLEW_RATE_MIN},									// 93
	{SLEW_RATE_MIN, INT16U_MAX, SLEW_RATE_MIN},									// 94
	{SLEW_RATE_MIN, INT16U_MAX, SLEW_RATE_MIN},									// 95
	{SLEW_RATE_MIN, INT16U_MAX, SLEW_RATE_MIN},									// 96
	{SLEW_RATE_MIN, INT16U_MAX, SLEW_RATE_MIN},									// 97
	{0, INT16U_MAX, COEF_FLATTOP_DURATION_DEF},									// 98
	{0, INT16U_MAX, COEF_FLATTOP_DURATION_DEF},									// 99
	{0, COEF_THRESHOLD_MAX, 0},													// 100
	{R_TESTLOAD_MIN, INT16U_MAX, R_TESTLOAD_DEF},								// 101
	{0, 0, 0},																	// 102
	{0, 0, 0},																	// 103
	{0, 0, 0},																	// 104
	{0, 0, 0},																	// 105
	{NO, YES, NO},																// 106
	{NO, YES, NO},																// 107
	{0, 0, 0},																	// 108
	{0, 0, 0},																	// 109
	{0, 0, 0},																	// 110
	{0, 0, 0},																	// 111
	{0, 0, 0},																	// 112
	{0, 0, 0},																	// 113
	{0, 0, 0},																	// 114
	{0, 0, 0},																	// 115
	{0, 0, 0},																	// 116
	{0, 0, 0},																	// 117
	{0, 0, 0},																	// 118
	{0, 0, 0},																	// 119
	{0, 0, 0},																	// 120
	{0, 0, 0},																	// 121
	{0, 0, 0},																	// 122
	{0, 0, 0},																	// 123
	{0, 0, 0},																	// 124
	{0, 0, 0},																	// 125
	{0, 0, 0},																	// 126
	{0, 0, 0}																	// 127
};

const TableItemConstraint VConstraint[DATA_TABLE_WP_START - DATA_TABLE_WR_START] =
{
	{U_IGES_MIN, U_IGES_MAX, U_IGES_DEF},										// 128
	{I_UGETH_MIN, I_UGETH_MAX, I_UGETH_DEF},									// 129
	{NO, YES, NO},																// 130
	{0, (COMMUTATION_TABLE_SIZE - 1), 0},										// 131
	{0, INT16U_MAX, 0},															// 132
	{0, 0, 0},																	// 133
	{0, 0, 0},																	// 134
	{0, 0, 0},																	// 135
	{0, 0, 0},																	// 136
	{0, 0, 0},																	// 137
	{0, 0, 0},																	// 138
	{0, 0, 0},																	// 139
	{0, 0, 0},																	// 140
	{0, 0, 0},																	// 141
	{0, 0, 0},																	// 142
	{0, 0, 0},																	// 143
	{0, 0, 0},																	// 144
	{0, 0, 0},																	// 145
	{0, 0, 0},																	// 146
	{0, 0, 0},																	// 147
	{0, 0, 0},																	// 148
	{0, 0, 0},																	// 149
	{0, INT16U_MAX, 0},															// 150
	{NO, I_CHANNEL_7, NO},														// 151
	{0, 0, 0},																	// 152
	{0, 0, 0},																	// 153
	{0, 0, 0},																	// 154
	{0, 0, 0},																	// 155
	{0, 0, 0},																	// 156
	{0, 0, 0},																	// 157
	{0, 0, 0},																	// 158
	{0, 0, 0},																	// 159
	{0, 0, 0},																	// 160
	{0, 0, 0},																	// 161
	{0, 0, 0},																	// 162
	{0, 0, 0},																	// 163
	{0, 0, 0},																	// 164
	{0, 0, 0},																	// 165
	{0, 0, 0},																	// 166
	{0, 0, 0},																	// 167
	{0, 0, 0},																	// 168
	{0, 0, 0},																	// 169
	{0, 0, 0},																	// 170
	{0, 0, 0},																	// 171
	{0, 0, 0},																	// 172
	{0, 0, 0},																	// 173
	{0, 0, 0},																	// 174
	{0, 0, 0},																	// 175
	{0, 0, 0},																	// 176
	{0, 0, 0},																	// 177
	{0, 0, 0},																	// 178
	{0, 0, 0},																	// 179
	{0, 0, 0},																	// 180
	{0, 0, 0},																	// 181
	{0, 0, 0},																	// 182
	{0, 0, 0},																	// 183
	{0, 0, 0},																	// 184
	{0, 0, 0},																	// 185
	{0, 0, 0},																	// 186
	{0, 0, 0},																	// 187
	{0, 0, 0},																	// 188
	{0, 0, 0},																	// 189
	{0, 0, 0},																	// 190
	{0, 0, 0}																	// 191
};
